...
function calledAfter(source, end) {
  return /^\s*(?:<[^>\n]+>\s*)?\(/.test(source.slice(end, end + 160));
}

function memberRole(source, end) {
  return calledAfter(source, end) ? codeRoles.method : codeRoles.property;
}
 ...
 * keep the same binding/property roles they have outside a string. */
  ...
  for (const string of source.matchAll(pattern)) {
    for (const expression of string[0].matchAll(/\{([A-Za-z_][\w']*(?:\.[A-Za-z_][\w']*)*)\}/g)) {
      ...
      for (const [index, name] of names.entries()) {
        const role = index > 0 || members.has(name) ? codeRoles.property : codeRoles.binding;
        spans.add(cursor, cursor + name.length, role, 80);
        cursor += name.length;
...
function refineFsharp(source) {
  ...
  lines(masked, (line, offset) => {
    ...
    if (match) {
      ...
      spans.add(at, at + match[1].length, codeRoles.type, 65); types.add(match[1]);
    }
    match = /^\s*\|\s*([A-Z][\w']*)/.exec(line);
    if (match) {
      const at = offset + line.indexOf(match[1]);
      spans.add(at, at + match[1].length, codeRoles.enumMember, 65); enumMembers.add(match[1]);
    }
    for (const item of line.matchAll(/\|\s*([A-Z][\w']*)/g)) {
      const at = offset + item.index + item[0].lastIndexOf(item[1]);
      spans.add(at, at + item[1].length, codeRoles.enumMember, 65); enumMembers.add(item[1]);
    }
    for (const item of line.matchAll(/(?:\{|;)\s*([A-Z][\w']*)\s*:/g)) {
      const at = offset + item.index + item[0].lastIndexOf(item[1]);
      spans.add(at, at + item[1].length, codeRoles.property, 65); properties.add(item[1]);
    }
    match = /^\s*([A-Z][\w']*)\s*:/.exec(line);
    if (match) {
      const at = offset + line.indexOf(match[1]);
      spans.add(at, at + match[1].length, codeRoles.property, 65); properties.add(match[1]);
    }
    match = /^\s*let\s+(?:rec\s+)?([A-Za-z_][\w']*)([^=]*)=/.exec(line);
    if (match) {
      const nameAt = offset + line.indexOf(match[1]);
      const callable = match[2].trim().length > 0;
      spans.add(nameAt, nameAt + match[1].length,
        callable ? codeRoles.functionDeclaration : codeRoles.binding, 65);
      (callable ? functions : bindings).add(match[1]);
      const argsAt = line.indexOf(match[2], line.indexOf(match[1]) + match[1].length);
      for (const arg of match[2].matchAll(/[a-z_][\w']*/g)) {
        spans.add(offset + argsAt + arg.index, offset + argsAt + arg.index + arg[0].length, codeRoles.binding, 60);
        bindings.add(arg[0]);
      }
    }
    for (const item of line.matchAll(/\.([A-Za-z_][\w']*)/g)) {
      if (/^\s*(?:namespace|open)\b/.test(line)) continue;
      const start = offset + item.index + 1;
      spans.add(start, start + item[1].length, memberRole(source, start + item[1].length), 70);
    }
  });
  for (const match of masked.matchAll(/\b[A-Za-z_][\w']*\b/g)) {
    const lineStart = masked.lastIndexOf('\n', match.index) + 1;
    if (/^\s*(?:namespace|open|module)\b/.test(masked.slice(lineStart, match.index))) continue;
    if (types.has(match[0])) spans.add(match.index, match.index + match[0].length, codeRoles.type, 35);
    else if (enumMembers.has(match[0])) spans.add(match.index, match.index + match[0].length, codeRoles.enumMember, 35);
    else if (properties.has(match[0])) spans.add(match.index, match.index + match[0].length, codeRoles.property, 35);
    else if (functions.has(match[0])) spans.add(match.index, match.index + match[0].length, codeRoles.method, 35);
    else if (bindings.has(match[0])) spans.add(match.index, match.index + match[0].length, codeRoles.binding, 30);
  }
  wordSpans(spans, masked, new Set(['namespace', 'open', 'type', 'of', 'module', 'let', 'rec', 'function', 'match', 'with', 'if', 'then', 'else', 'for', 'in', 'do', 'yield', 'return']), codeRoles.keyword, 45);
  ...
  addBraceInterpolations(spans, source, /\$@?"(?:""|\\.|[^"\\])*(?:"|$)/g, properties);
  return spans.finish();
}
...
function refineShader(source, language) {
  ...
  lines(masked, (line, offset) => {
    ...
    if (aggregate) {
      ...
      spans.add(at, at + aggregate[1].length, codeRoles.type, 70); typeNames.add(aggregate[1]);
      ...
      if (inlineBody) {
        ...
        for (const member of inlineBody[1].matchAll(/\b[A-Za-z_]\w*(?:<[^>]+>)?\s+([A-Za-z_]\w*)\s*(?::|;)/g)) {
          ...
          spans.add(memberAt, memberAt + member[1].length, codeRoles.property, 78);
    ...
    if (functionMatch && !['if', 'for', 'while', 'switch'].includes(functionMatch[1])) {
      ...
      spans.add(nameAt, nameAt + functionMatch[2].length, codeRoles.functionDeclaration, 75); functions.add(functionMatch[2]);
      if (functionMatch[4]) {
      ...
      for (const param of functionMatch[3].matchAll(/(?:\b[A-Za-z_]\w*(?:<[^>]+>)?\s+)([A-Za-z_]\w*)/g)) {
        ...
        spans.add(at, at + param[1].length, codeRoles.binding, 70); locals.add(param[1]);
      }
    ...
    if (declaration && !functionMatch && !aggregate) {
      const at = offset + line.indexOf(declaration[2], line.indexOf(declaration[1]) + declaration[1].length);
      const role = inAggregate
        ? codeRoles.property
        : inFunction
          ? codeRoles.binding
          : language === 'shaderlab' && /^_/.test(declaration[2])
            ? 'purple'
            : codeRoles.global;
    ...
    for (const property of line.matchAll(/\.([A-Za-z_]\w*)/g)) {
      const start = offset + property.index + 1;
      spans.add(start, start + property[1].length, memberRole(source, start + property[1].length), 75);
  ...
  for (const match of masked.matchAll(/\b[A-Za-z_]\w*\b/g)) {
    if (typeNames.has(match[0])) spans.add(match.index, match.index + match[0].length, codeRoles.type, 55);
    else if (members.has(match[0])) spans.add(match.index, match.index + match[0].length,
      memberRole(source, match.index + match[0].length), 35);
    else if (locals.has(match[0])) spans.add(match.index, match.index + match[0].length, codeRoles.binding, 35);
    else if (globals.has(match[0])) spans.add(match.index, match.index + match[0].length, codeRoles.global, 35);
  }
  for (const match of masked.matchAll(/\b([A-Za-z_]\w*)\s*\(/g)) {
    if (!typeNames.has(match[1]) && match[1] !== 'register') {
  ...
  wordSpans(spans, masked, new Set(['struct', 'cbuffer', 'return', 'if', 'else', 'for', 'while', 'register',
    'Shader', 'Properties', 'SubShader', 'Tags', 'LOD', 'Pass', 'CGPROGRAM', 'ENDCG', 'pragma', 'include']), codeRoles.keyword, 45);
  wordSpans(spans, masked, new Set(['void']), codeRoles.keyword, 60);
...
function refinePowerShell(source) {
  ...
  let depth = 0;
  for (const match of masked.matchAll(/\[([^\]]+)\]/g)) {
    spans.add(match.index + 1, match.index + match[0].length - 1, codeRoles.type, 60);
  }
  for (const match of masked.matchAll(/\bclass\s+([A-Za-z_]\w*)/gi)) {
    const at = match.index + match[0].lastIndexOf(match[1]);
    spans.add(at, at + match[1].length, codeRoles.type, 70);
  }
  for (const match of masked.matchAll(/\bfunction\s+([A-Za-z_][\w-]*)/gi)) {
    const at = match.index + match[0].lastIndexOf(match[1]);
    spans.add(at, at + match[1].length, codeRoles.functionDeclaration, 70); functions.add(match[1].toLowerCase());
  }
  lines(masked, (line, offset) => {
    ...
    if (match && inClass) {
      ...
      spans.add(at - 1, at + match[1].length, codeRoles.property, 70); members.add(match[1].toLowerCase());
    }
    match = /^\s*(?:\[[^\r\n]+\]\s+)?([A-Za-z_]\w*)\s*\(/.exec(line);
    ...
    for (const key of line.matchAll(/^\s*([A-Za-z_]\w*)\s*=/g)) {
      ...
      spans.add(at, at + key[1].length, codeRoles.property, 65);
    }
  ...
  for (const match of source.matchAll(/\.([A-Za-z_]\w*)/g)) {
    ...
    spans.add(match.index + 1, match.index + match[0].length,
      memberRole(source, match.index + match[0].length), 75);
  }
  for (const match of masked.matchAll(/\$[A-Za-z_]\w*/g)) {
    spans.add(match.index, match.index + match[0].length, codeRoles.binding, 45);
  }
  for (const string of source.matchAll(/"(?:`.|[^"`])*(?:"|$)/g)) {
    for (const match of string[0].matchAll(/\$\{([A-Za-z_]\w*)\}|\$([A-Za-z_]\w*)/g)) {
      ...
      if (match[1]) {
        ...
        spans.add(at + 2, at + 2 + name.length, codeRoles.binding, 72);
        spans.add(at + match[0].length - 1, at + match[0].length, 'green', 70);
      } else spans.add(at, at + match[0].length, codeRoles.binding, 72);
    }
  }
  for (const match of masked.matchAll(/::([A-Za-z_]\w*)/g)) {
    const at = match.index + 2;
    spans.add(at, at + match[1].length, memberRole(source, at + match[1].length), 70);
  }
  for (const match of masked.matchAll(/\b[A-Za-z_][\w-]*\b/g)) {
    if (functions.has(match[0].toLowerCase()) && !/\bfunction\s+$/i.test(masked.slice(Math.max(0, match.index - 12), match.index))) {
      spans.add(match.index, match.index + match[0].length, codeRoles.method, 45);
    }
  }
  wordSpans(spans, masked, new Set(['class', 'function', 'param', 'process', 'return', 'if', 'else', 'foreach', 'in', 'switch']), codeRoles.keyword, 50);
...
function refinePerl(source, language) {
  ...
  for (const match of masked.matchAll(/\b(method|sub)\s+([A-Za-z_][\w-]*)/g)) {
    ...
    spans.add(at, at + match[2].length, member ? codeRoles.method : codeRoles.functionDeclaration, 70);
  ...
  for (const match of masked.matchAll(variablePattern)) {
    ...
    spans.add(match.index, match.index + match[0].length,
      member ? codeRoles.property : codeRoles.binding, 60);
  }
  for (const string of source.matchAll(/"(?:\\.|[^"\\])*(?:"|$)/g)) {
    for (const match of string[0].matchAll(variablePattern)) {
      ...
      spans.add(string.index + match.index, string.index + match.index + match[0].length,
        member ? codeRoles.property : codeRoles.binding, 70);
    }
  }
  for (const match of masked.matchAll(/(?:->|\.)([A-Za-z_][\w-]*)/g)) {
    const lineStart = masked.lastIndexOf('\n', match.index) + 1;
  ...
  for (const match of masked.matchAll(/\b([A-Za-z_]\w*)\s*=>/g)) {
    spans.add(match.index, match.index + match[1].length, codeRoles.namedArgument, 65);
  }
  for (const match of source.matchAll(/(?:->)?\{([A-Za-z_]\w*)\}/g)) {
    const at = match.index + match[0].lastIndexOf(match[1]);
    spans.add(at, at + match[1].length, codeRoles.property, 70);
  }
  ...
  if (raku) {
    for (const match of masked.matchAll(/\benum\s+[A-Za-z_]\w*\s+<([^>]*)>/g)) {
      ...
      for (const value of match[1].matchAll(/[A-Za-z_]\w*/g)) {
        spans.add(valuesAt + value.index, valuesAt + value.index + value[0].length,
  ...
  wordSpans(spans, masked, new Set(['package', 'class', 'enum', 'has', 'is', 'required', 'method', 'sub', 'my', 'our', 'use', 'return', 'if', 'else', 'given', 'when', 'default', 'new']), codeRoles.keyword, 50);
  wordSpans(spans, masked, new Set(['False', 'True']), 'yellow', 50);
  wordSpans(spans, masked, new Set(raku ? ['Str', 'Array', 'Bool', 'Seq', 'Int'] : ['scalar']), codeRoles.type, 55);
  for (const match of masked.matchAll(/\b\d+(?:\.\d+)?\b/g)) spans.add(match.index, match.index + match[0].length, 'orange', 45);
  return spans.finish();
}
...
function refineVisualBasic(source) {
  ...
  for (const match of masked.matchAll(/\bProperty[ \t]+([A-Za-z_]\w*)/gi)) {
    const at = match.index + match[0].lastIndexOf(match[1]);
    spans.add(at, at + match[1].length, codeRoles.property, 70);
  }
  for (const match of masked.matchAll(/\b(?:Function|Sub)[ \t]+([A-Za-z_]\w*)/gi)) {
    const at = match.index + match[0].lastIndexOf(match[1]);
    spans.add(at, at + match[1].length, codeRoles.method, 70); members.add(match[1].toLowerCase());
  }
  for (const match of masked.matchAll(/\b(?:Function|Sub)[ \t]+[A-Za-z_]\w*\s*\(([^)]*)\)/gi)) {
    ...
    for (const parameter of match[1].matchAll(/(?:^|,)\s*(?:Optional\s+)?([A-Za-z_]\w*)\s+As\b/gi)) {
      ...
      spans.add(at, at + parameter[1].length, codeRoles.binding, 70);
  ...
  for (const match of masked.matchAll(/\bDim\s+([A-Za-z_]\w*)|\bFor\s+Each\s+([A-Za-z_]\w*)/gi)) {
    ...
    spans.add(at, at + name.length, codeRoles.binding, 70);
  ...
  for (const match of masked.matchAll(/\b([A-Za-z_]\w*)\s*\(/g)) {
    if (members.has(match[1].toLowerCase())) {
      spans.add(match.index, match.index + match[1].length, codeRoles.method, 60);
    }
  }
  for (const string of source.matchAll(/\$"(?:""|[^"])*(?:"|$)/g)) {
    ...
    for (const expression of string[0].matchAll(/\{([A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*)\}/g)) {
      ...
      for (const [index, name] of expression[1].split('.').entries()) {
        ...
        const role = index > 0 || members.has(lower)
          ? codeRoles.property
          : bindings.has(lower)
            ? codeRoles.binding
            : codeRoles.property;
  ...
  wordSpans(spans, masked, new Set(['String', 'Boolean', 'Integer', 'Long', 'Double', 'Single', 'Object', 'Void']), codeRoles.type, 60);
  wordSpans(spans, masked, new Set(['Imports', 'Public', 'Private', 'Protected', 'Friend', 'ReadOnly', 'NotInheritable', 'Class', 'Module', 'Property', 'Function', 'Sub', 'New', 'As', 'Of', 'Return', 'Iterator', 'For', 'Each', 'In', 'If', 'Then', 'Else', 'Yield', 'End', 'Dim', 'Optional']), codeRoles.keyword, 50);
  wordSpans(spans, masked, new Set(['True', 'False']), 'yellow', 50);
  for (const match of masked.matchAll(/\b\d+(?:\.\d+)?\b/g)) spans.add(match.index, match.index + match[0].length, 'orange', 45);
  return spans.finish();
}
...
function refineHandlebars(source) {
  ...
  for (const match of source.matchAll(/\{\{\{?[\s\S]*?\}\}\}?/g)) {
    ...
    for (const name of body.matchAll(/@?[A-Za-z_][\w-]*/g)) {
      ...
      let role = previous || hashKey ? 'purple' : aliases.has(name[0]) ? codeRoles.binding : 'yellow';
...
function refinePug(source) {
  ...
  lines(source, (line, offset) => {
    ...
    if (match) {
      ...
      spans.add(nameAt, nameAt + match[1].length, codeRoles.method, 75);
      ...
      for (const arg of match[2].matchAll(/[A-Za-z_]\w*/g)) {
        aliases.add(arg[0]);
        spans.add(argsAt + arg.index, argsAt + arg.index + arg[0].length, codeRoles.binding, 75);
      }
      return;
    }
    ...
    if (match) {
      ...
      for (const name of match[1].matchAll(/[A-Za-z_]\w*/g)) {
        aliases.add(name[0]);
        spans.add(namesAt + name.index, namesAt + name.index + name[0].length, codeRoles.binding, 75);
      }
    ...
    for (const name of aliases) {
      for (const use of body.matchAll(new RegExp(`\\b${name}\\b`, 'g'))) {
        ...
        if (!/\.\s*$/.test(before) && !/[\w+-]/.test(adjacent)) spans.add(offset + leading + use.index,
          offset + leading + use.index + name.length, codeRoles.binding, 78);
      }
    }
  });
  return spans.finish();
  ...
  for (const match of masked.matchAll(/^\s*[A-Za-z_]\w*\s*:\s*\(([^)]*)\)\s*[-=]>/gm)) {
    ...
    for (const parameter of match[1].matchAll(/[A-Za-z_]\w*/g)) {
      spans.add(parametersAt + parameter.index,
        parametersAt + parameter.index + parameter[0].length, codeRoles.binding, 72);
      bindings.add(parameter[0]);
    }
  }
  for (const match of source.matchAll(/@[A-Za-z_]\w*/g)) {
    spans.add(match.index, match.index + match[0].length, codeRoles.property, 80);
  }
  for (const match of masked.matchAll(/\.([A-Za-z_]\w*)/g)) {
    spans.add(match.index + 1, match.index + match[0].length,
      memberRole(source, match.index + match[0].length), 70);
  }
  for (const match of masked.matchAll(/^\s*([A-Za-z_]\w*)\s*=|\bfor\s+([A-Za-z_]\w*)\s+in\b/gm)) {
    const name = match[1] ?? match[2];
    const at = match.index + match[0].indexOf(name);
    spans.add(at, at + name.length, codeRoles.binding, 65);
    bindings.add(name);
  }
  for (const match of masked.matchAll(/\b[A-Za-z_]\w*\b/g)) {
    if (functions.has(match[0])) {
      spans.add(match.index, match.index + match[0].length, codeRoles.method, 45);
    } else if (bindings.has(match[0])) {
      spans.add(match.index, match.index + match[0].length, codeRoles.binding, 45);
    }
  }
  for (const match of masked.matchAll(/\bnull\b/g)) {
    spans.add(match.index, match.index + match[0].length, 'orange', 80);
  }
  for (const match of masked.matchAll(/\b\d+(?:\.\d+)?\b/g)) {
    spans.add(match.index, match.index + match[0].length, 'orange', 70);
  }
...
function refineClojure(source) {
  ...
  for (const match of masked.matchAll(/\((ns|defrecord|defprotocol|def|defn)\s+([^\s()[\]{}]+)/g)) {
    const name = match[2];
    const at = match.index + match[0].lastIndexOf(name);
    const role = ['defrecord', 'defprotocol'].includes(match[1])
      ? codeRoles.type
      : match[1] === 'defn'
        ? codeRoles.functionDeclaration
        : codeRoles.binding;
    spans.add(at, at + name.length, role, 75);
    ...
    if (match[1] === 'defrecord') {
      ...
      if (vector) for (const field of vector[1].matchAll(/[A-Za-z_][\w-]*/g)) {
        ...
        spans.add(fieldAt, fieldAt + field[0].length, codeRoles.property, 70);
      }
    }
  }
  for (const match of masked.matchAll(/\((?:defn|fn)\s+[^\s()[\]{}]+\s+\[([^\]]*)\]/g)) {
    ...
    for (const name of parameters.matchAll(/[A-Za-z_][\w-]*/g)) {
      spans.add(parametersAt + name.index, parametersAt + name.index + name[0].length, codeRoles.binding, 70);
      addLocal(name[0], match.index);
    }
  }
  for (const match of masked.matchAll(/\(render\s+\[\{[^}]*\}\s+([A-Za-z_][\w-]*)\]/g)) {
    const at = match.index + match[0].lastIndexOf(match[1]);
    spans.add(at, at + match[1].length, codeRoles.binding, 70);
  ...
  for (const match of masked.matchAll(/:keys\s+\[([^\]]*)\]|when-let\s+\[([A-Za-z_][\w-]*)/g)) {
    ...
    for (const name of names.matchAll(/[A-Za-z_][\w-]*/g)) {
      spans.add(namesAt + name.index, namesAt + name.index + name[0].length, codeRoles.binding, 70);
      addLocal(name[0], match.index);
    }
  }
  for (const match of masked.matchAll(/:[A-Za-z_][\w-]*/g)) {
    spans.add(match.index, match.index + match[0].length, codeRoles.property, 60);
  }
  for (const match of masked.matchAll(/\(\s*([^\s()[\]{}]+)/g)) {
    const symbol = match[1];
    const at = match.index + match[0].lastIndexOf(symbol);
    if (symbol.startsWith(':')) spans.add(at, at + symbol.length, codeRoles.property, 72);
    else if (special.has(symbol)) spans.add(at, at + symbol.length, codeRoles.keyword, 70);
    else if (symbol.includes('/')) {
      const slash = symbol.indexOf('/');
      spans.add(at, at + slash, codeRoles.binding, 65);
  ...
  for (const match of masked.matchAll(/\((?:map|filter|remove|keep|reduce)\s+([A-Za-z_][\w?!-]*)/g)) {
    const at = match.index + match[0].lastIndexOf(match[1]);
    spans.add(at, at + match[1].length, codeRoles.method, 72);
  }
  for (const match of masked.matchAll(/\b[A-Za-z_][\w-]*\b/g)) {
    if (localBindings.some((binding) => binding.name === match[0] && binding.start <= match.index && binding.end >= match.index)) {
      ...
      spans.add(match.index, match.index + match[0].length, codeRoles.type, 50);
